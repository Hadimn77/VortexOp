    ...
    try:
        if skip_preprocessing:
            ...
            if lattice_model:
                ...
                log_func("Step 5/6: Ensuring mesh is watertight with PyMeshFix...", percent=40)
                ...
                # Step 2: Convert to Trimesh for component analysis
                log_func("Step 2/6: Analyzing mesh components...", percent=20)
                faces = surface_mesh.faces.reshape(-1, 4)[:, 1:]
                trimesh_mesh = trimesh.Trimesh(vertices=surface_mesh.points, faces=faces)

                # Step 3: Isolate the largest component FIRST to remove junk geometry
                ...
                # Step 4: Convert back to PyVista for robust repair
                faces_padded = np.hstack([np.full((trimesh_mesh.faces.shape[0], 1), 3), trimesh_mesh.faces])
                main_component_surface = pv.PolyData(trimesh_mesh.vertices, faces_padded)

                # Step 5: Run PyMeshFix repair on the ISOLATED main component
        ...
        # This final check for manifold is now more reliable
